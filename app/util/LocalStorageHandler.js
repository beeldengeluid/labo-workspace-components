...
export default class LocalStorageHandler {
  ...
  static pushItemToLocalStorage = (key, item, itemIdentifier = null) => {
    if (LocalStorageHandler.checkLocalStorageKey(key, false)) {
      const currentDataOnStorage =
        LocalStorageHandler.getJSONFromLocalStorage(key);
      let obj = null;
      if (currentDataOnStorage === null) {
        obj = [item];
        this.storeJSONInLocalStorage(key, obj);
      } else {
        // if item is not in array then push it.
        if (
          !currentDataOnStorage.find((val) => {
            if (itemIdentifier) {
        ...
        this.storeJSONInLocalStorage(key, currentDataOnStorage);
      }
    }
    return null;
  };
  ...
  static removeItemInLocalStorage = (key, item, itemIdentifier = null) => {
    if (LocalStorageHandler.checkLocalStorageKey(key, false)) {
      const currentDataOnStorage =
        LocalStorageHandler.getJSONFromLocalStorage(key);
      ...
      if (currentDataOnStorage.length > 0) {
        ...
        this.removeJSONByKeyInLocalStorage(key);
      }
    }
  };
